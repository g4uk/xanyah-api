...
RSpec.describe 'Shippings', type: :request do
  ...
  describe 'GET /shippings' do
    it 'returns only permitted shippings' do
      ...
      get shippings_path, params: {headers: user.create_new_auth_token}
      expect(response).to have_http_status(:ok)
      expect(JSON.parse(response.body).size).to eq(1)
    end

    it 'return empty if !membership' do
      get shippings_path, params: {headers: create(:user).create_new_auth_token}
      expect(response).to have_http_status(:ok)
  ...
  describe 'GET /shippings/:id' do
    it 'returns shipping if membership' do
      ...
      get shipping_path(shipping), params: {headers: user.create_new_auth_token}
      expect(response).to have_http_status(:ok)
      expect(JSON.parse(response.body)['id']).to be_present
    end

    it 'returns 401 if !membership' do
      get shipping_path(create(:shipping)), params: {headers: create(:user).create_new_auth_token}
      expect(response).to have_http_status(:unauthorized)
    end
  ...
  describe 'PATCH /shippings/:id/lock' do
    it 'updates shipping if membership' do
      ...
      patch lock_shipping_path(shipping), params: {headers: user.create_new_auth_token}
      expect(response).to have_http_status(:ok)
      expect(JSON.parse(response.body)['locked_at']).not_to eq(nil)
    end

    it 'returns 401 if !membership' do
      patch lock_shipping_path(create(:shipping)), params: {headers: create(:user).create_new_auth_token}
      expect(response).to have_http_status(:unauthorized)
    end
  ...
  describe 'DELETE /shippings/:id' do
    it 'deletes shipping variant if membership >= admin' do
      store_membership.update(role: :admin)
      shipping = create(:shipping, store: store, locked_at: nil)
      delete shipping_path(shipping), params: {headers: store_membership.user.create_new_auth_token}
    ...
    it 'returns 401 if !membership' do
      delete shipping_path(create(:shipping)), params: {headers: create(:user).create_new_auth_token}
      expect(response).to have_http_status(:unauthorized)
      expect(JSON.parse(response.body)).to have_key('errors')
    end
